...
function createAction() {
  ...
  useRipeMaterialRepaint().createOrder(input)
    .then(() => {
...
function receiveAction() {
  ...
  useRipeMaterialRepaint().receive(selectedData.value.id, input)
    .then(() => {
...
function deleteAction() {
  ...
  paintLoading.value = true;
  useRipeMaterialRepaint().deleteOrder(selectedData.value.id)
    .then(() => {
      ...
      $q.notify({
        ...
        position: 'top',
        timeout: 1000,
        message: t('forms.repaint.confirmation.successOrderDeleted')
      });
      clearAction();
      emit('submit');
    })
