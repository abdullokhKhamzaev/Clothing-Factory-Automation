  ...
  <q-card flat bordered class="q-pa-sm text-green q-mb-md">
    ...
    >
      <q-card>
          ...
          <div v-for="(count, modelName) in modelsStats.stats" :key="modelName">
            ...
            >
              <q-card class="q-ml-md">
                <div v-for="(userCount, sewerName) in modelsStats.statsByUser[modelName]" :key="sewerName">
        ...
        <!-- User View -->
        <div v-else>
          <div v-for="(count, sewerName) in modelsStats.userStats" :key="sewerName">
            <q-expansion-item
              :label="`${sewerName} (Jami: ${count})`"
              icon="person"
              header-class="text-secondary text-weight-medium"
            >
              <q-card class="q-ml-md">
                <div v-for="(modelCount, modelName) in modelsStats.statsByModel[sewerName]" :key="modelName">
                  <q-splitter v-model="splitterModel">
                    <template v-slot:before>
                      <q-card-section class="q-pl-md">
