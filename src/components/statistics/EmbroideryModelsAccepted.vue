...
<template>
  <q-card flat bordered class="q-pa-sm text-green q-mb-md">
    <q-card-section class="flex justify-between">
      <div class="text-h6 text-primary">Vishivka urilgan mahsulotlar:</div>
    ...
    >
      <q-card>
          ...
          <div v-for="(count, modelName) in modelsStats.stats" :key="modelName">
            ...
            >
              <q-card class="q-ml-md">
                <div v-for="(userCount, embroidererName) in modelsStats.statsByUser[modelName]" :key="embroidererName">
        ...
        <!-- User View -->
        <div v-else>
          <div v-for="(count, embroidererName) in modelsStats.userStats" :key="embroidererName">
            <q-expansion-item
              :label="`${embroidererName} (Jami: ${count})`"
              icon="person"
              header-class="text-secondary text-weight-medium"
            >
              <q-card class="q-ml-md">
                <div v-for="(modelCount, modelName) in modelsStats.statsByModel[embroidererName]" :key="modelName">
                  <q-splitter v-model="splitterModel">
                    <template v-slot:before>
                      <q-card-section class="q-pl-md">
