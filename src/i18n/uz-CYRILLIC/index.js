export default {
  statistics: {
    modelView: 'Маҳсулот бўйича',
    materialView: 'Материал бўйича',
    userView: 'Ходим бўйича',
    customerView: 'Харидор бўйича',
    additionalInfo: 'Қўшимча маълумотлар',
    total: 'Жами',
    roll: 'рўлон',
    and: 'ва',
    sort2: '2-сорт',
    weave: {
      done: 'Тўқилди',
      donePending: 'Тўқилди (кутилмоқда)',
      title: 'Тўқилган маҳсулотлар:',
      titlePending: 'Тўқилган маҳсулотлар (кутилмоқда):',
    },
    sale: {
      salesTab: 'Савдо',
      analyticsTab: 'Таҳлил',
      title: 'Маҳсулотлар савдоси:',
    },
    analytics: {
      title: 'Савдо таҳлили:',
      sellThrough: 'Сотув / Ишлаб чиқариш нисбати',
      sellThroughCaption: 'Танланган даврда ишлаб чиқарилганнинг неча фоизи сотилди',
      produced: 'Ишлаб чиқарилди',
      sold: 'Сотилди',
      notProducedInPeriod: 'бу даврда ишлаб чиқарилмаган',
      topModels: 'Энг кўп сотилаётган моделлар',
      topModelsCaption: 'Умумий савдодаги улуши билан',
      concentration: 'Харидор концентрацияси',
      concentrationCaption: 'Битта харидор модель савдосининг қанча қисмини банд қилган',
      topCustomers: 'Энг катта харидорлар',
      topCustomersCaption: 'Дона ва сумма бўйича рейтинг',
      stock: 'Омбордаги қолдиқ',
      stockCaption: 'Тайёр маҳсулот омборининг ҳозирги ҳолати (санага боғлиқ эмас)',
      stockEmpty: 'Омборда маҳсулот йўқ',
      noData: 'Бу даврда маълумот йўқ',
      noSales: 'Бу даврда савдо йўқ',
      notSoldInPeriod: 'бу даврда сотилмаган',
      soldBadge: 'сотилди',
      totalStock: 'Жами қолдиқ',
      totalSold: 'Жами сотилди',
      totalProduced: 'Жами ишлаб чиқарилди (Қадоқлаш)',
      debtors: 'Қарздорлар',
      debtorsCaption: 'Ҳозирги қарздор харидорлар (санага боғлиқ эмас)',
      debtorsEmpty: 'Қарздорлар йўқ',
      debtSince: 'Энг эски қарз',
    },
    demand: {
      tab: 'Талаб',
      title: 'Харидорлар талаби:',
      note: 'Буюртмалар бўлимига киритилган закалар асосида',
      demandVsDelivered: 'Талаб / Етказиб бериш',
      demandVsDeliveredCaption: 'Заказ қилинган ва шу даврда сотилган миқдорлар',
      demand: 'Талаб',
      delivered: 'Етказилди',
      unmet: 'Етказилмади',
      fulfillment: 'Қондириш',
      topModels: 'Энг талабгор моделлар',
      topModelsCaption: 'Заказлардаги умумий талабдаги улуши билан',
      byCustomers: 'Харидорлар кесимида',
      byCustomersCaption: 'Ҳар бир харидорнинг талаби ва унга етказилгани',
      noData: 'Бу даврда буюртма йўқ',
      totalDemand: 'Жами талаб',
      totalDelivered: 'Жами етказилди',
      totalUnmet: 'Жами етказилмади',
    },
    timing: {
      confirm: 'Тасдиқлаш вақти',
      waiting: 'Кутиш вақти',
      avg: 'ўртача',
      max: 'энг узоқ',
      oldest: 'энг эскиси',
      day: 'кун',
      hour: 'соат',
      minute: 'дақиқа',
      lessThanMinute: '1 дақиқадан кам',
      details: 'Батафсил (вақтлар)',
      submitted: 'Топширилди',
      confirmed: 'Тасдиқланди',
      waitingFor: 'Кутмоқда',
    },
    pipeline: {
      title: 'Ишлаб чиқариш жараёни',
      caption: 'Босқичма-босқич ҳолат — тасдиқланмаган ишлар тўқ сариқ рангда',
      confirmed: 'Тасдиқланган',
      pending: 'Тасдиқ кутмоқда',
      allConfirmed: 'Ҳаммаси тасдиқланган',
    },
    dashboard: {
      sold: 'Сотилди',
      produced: 'Ишлаб чиқарилди',
      stock: 'Омбордаги қолдиқ',
      revenue: 'Тушум',
      paid: 'Тўланган',
      debt: 'Қарз',
      expenses: 'Харажат',
      profit: 'Фойда (тахминий)',
      expensesNote: 'ойлик ва айирбошлашсиз',
    },
  },
  transaction: {
    productCost: {
      threadPurchased: 'Ип сотиб олинди',
      payedForThreadPurchaseDebt: 'Ип савдосининг қарзи учун тўланди',
      accessoryPurchased: 'Аксессуар сотиб олинди',
      payedForAccessoryDebt: 'Аксессуар савдосининг қарзи учун тўланди',
      materialPurchased: 'Материал сотиб олинди',
      payedForMaterialPurchaseDebt: 'Материал савдосининг қарзи учун тўланди',
      payedForPaintMaterial: 'Бўёқ учун тўланди',
      payedForPaintingDebt: 'Бўёқ савдосининг қарзи учун тўланди',
    },
    balanceIncreased: 'Баланс тўлдирилди',
    productSold: 'Маҳсулот сотилди',
    orderAdvance: 'Буюртма учун аванс',
    payedForIndebtedness: 'Қарздорлик учун тўланди',
    payedCustomerAdvance: 'Аванс берилди',
    payedCustomerSalary: 'Моаш берилди',
    receivedSaleDebt: 'Савдонинг қарзи қабул қилинди',
    tax: 'Солик',
    electricity: 'Свет',
    gas: 'Газ',
    water: 'Сув',
    repair: 'Тамирлаш',
    fare: 'Йўл киро',
    saleShare: 'Сотув улуш 3%',
    food: 'Озик-овкат',
    other: 'Бошка'
  },
  send: 'Юбориш',
  productCost: 'Махсулот тан нархига оид расходлар',
  periodCost: 'Махсулот тан нархига оид бўлмаган расходлар',
  paySalary: 'Моаш тўлаш',
  checkedBy: 'Текширилди',
  payAdvance: 'Аванс тўлаш',
  departure: 'Жўнав кетиш',
  oweUs: 'Биздан қарз',
  total: 'Жами',
  all: 'Барча',
  paid: 'Тўланди',
  logout: 'Чиқиш',
  salary: 'Моаш',
  add: 'Қўшиш',
  accessory: 'Аксессуар',
  indebtedness: 'Қарздорлик',
  exchanges: 'Пул алмашинуви',
  accept: 'Қабул қилиш',
  balance: 'Баланс',
  confirmed: 'Тасдиқланган',
  repaintOrders: 'Қайта бўйоқ буюртмалар',
  completedOrders: 'Якунланган буюртмалар',
  completedRepaintOrders: 'Якунланган қайта бўйоқ буюртмалар',
  convert: 'Айирбошлаш',
  columns: 'Устунлар',
  colors: 'Ранглар',
  confirm: 'Тасдиқлаш',
  delete: 'Ўчириш',
  debts: 'Қарзлар',
  edit: 'Таҳрирлаш',
  finish: 'Тугатиш',
  fabrics: 'Фабрикалар',
  from: 'Дан',
  inProgress: 'Жараёнда',
  language: 'Тил',
  materials: 'Материаллар',
  roll: 'Рўлон',
  noData: 'Маълумот мавжуд эмас',
  orders: 'Буюртмалар',
  pending: 'Кутилмоқда',
  reject: 'Рад этиш',
  report: 'Ҳисобот',
  pay: 'Тўлаш',
  to: 'Га',
  sell: 'Сотиш',
  thread: 'Ип',
  threads: 'Иплар',
  threadPurchase: 'Ип савдоси',
  ripeMaterialPurchase: 'Материал савдоси',
  ripeMaterialPaintPurchase: 'Боёк савдоси',
  transactions: 'Транзакциялар',
  sale: 'Савдо',
  quantity: 'Микдор',
  warehouse: 'Омбор',
  unripeMaterial: 'Тўкув материал',
  ripeMaterial: 'Материал',
  defects: 'Браглар',
  sendToEmbroidery: 'Вышивкага юбориш',
  sendToCutterDefective: 'Бичув (браг) га юбориш',
  sendToSewerWarehouse: 'Тикувга юбориш',
  sendToPackageWarehouse: 'Упаковкага юбориш',
  sendToProductsWarehouse: 'Складга юбориш',
  waiting: 'Кутилмоқда',
  waitingOrders: 'Кутилаётган буюртмалар',
  receive: 'Қабул қилиш',
  cutter: 'Бичувчи',
  cutterService: 'Бичувчи учун сервис',
  embroideryService: 'Вышивкачи учун сервис',
  embroidery: 'Вышивкачи',
  sewer: 'Тикувчи',
  sewerService: 'Тикувчи учун сервис',
  packager: 'Кадокловчи',
  packagerService: 'Кадокловчи учун сервис',
  monthly: 'Ойлик',
  service: 'Сервис',
  daily: 'Кунлик',
  made: 'Ишбай',
  piece: 'Дона',
  kg: 'КГ',
  meter: 'Метр',
  averagePrice: 'Ўртача нарх',
  workCount: 'Ишлар сони',
  quantityAndPrice: 'Миқдор ва Нарх',
  today: 'Бугун',
  totalPayment: 'Жами тўлов',
  paint: 'Бўёқ',
  paintFactory: 'Бўёқ фабрика',
  model: 'Модел',
  employees: 'Ходимлар',
  attendance: 'Давомат',
  ready: 'Тайёр',
  productNotFound: 'Хеч кандай махсулот топилмади.',
  isUsedRemainingMaterial: 'Қолдиқ материалдан фойдаландингизми?',
  roles: {
    ROLE_ADMIN: 'Админ',
    ROLE_USER: 'Фойдаланувчи',
    ROLE_WEAVER: 'Тўкувчи',
    ROLE_CUTTER: 'Бичувчи',
    ROLE_EMBROIDERER: 'Вышивкачи',
    ROLE_SEWER: 'Тикувчи',
    ROLE_PACKAGER: 'Упаковкачи',
    ROLE_MERCHANT: 'Савдогар',
    ROLE_MASTER: 'Мастер',
    ROLE_SUPER_ADMIN: 'Супер Админ'
  },
  statuses: {
    accepted: 'Қабул қилинган',
    notAccepted: 'Қабул қилинмаган',
    rejected: 'Рад этилган',
    confirmed: 'Тасдиқланган',
    pending: 'Кутилмоқда',
    completed: 'Якунланган',
    expected: 'Кутилаётган',
    received: 'Қабул қилинган',
    delivering: 'Етказилмоқда',
    delivered: 'Етказилди',
    cancelled: 'Бекор қилинди'
  },
  orderActions: {
    createTitle: 'Буюртма (заказ) яратиш',
    create: 'Буюртма яратиш',
    total: 'Жами',
    progress: 'Етказилиш',
    close: 'Етказилди деб ёпиш',
    cancel: 'Бекор қилиш',
    closeConfirm: 'Буюртма "Етказилди" деб ёпилсинми?',
    cancelConfirm: 'Буюртма бекор қилинсинми?',
    linkOrder: 'Буюртмага боғлаш (ихтиёрий)',
    fullyDelivered: 'Тўлиқ етказилди',
    closedPartial: 'Ёпилди (қисман)',
    deliveredSoFar: 'Етказилган',
    modelsNotInStock: 'Заказдаги баъзи моделлар омборда йўқ — улар автоматик қўшилмади',
    advance: 'Олдиндан тўлов (аванс)',
    advanceShort: 'Аванс',
    advanceRemainderShort: 'қолдиқ',
    advanceLeft: 'Аванс қолдиғи',
    useAdvance: 'Авансдан ишлатиш',
    advanceDepositFailed: 'Аванс кассага ёзилмади! Бюджет бўлимидан қўлда киритинг',
  },
  warehouses: {
    cutterWarehouse: 'Бичув омбор',
    cutterDefectiveWarehouse: 'Бичув (враг) омбор',
    embroideryWarehouse: 'Вышивка омбор',
    embroideryReadyWarehouse: 'Вышивка (тайёр) омбор',
    sewerWarehouse: 'Тикув омбор',
    sewerReadyWarehouse: 'Тикув (тайёр) омбор',
    packagerWarehouse: 'Упаковка омбор',
    packagerReadyWarehouse: 'Упаковка (тайёр) омбор',
    productsWarehouse: 'Склад',
  },
  // Бюджет компонентлари таржималари
  budgetStats: {
    title: 'Бюджет Статистикаси',
    somAccount: 'СЎМ Ҳисоб',
    usdAccount: 'USD Ҳисоб',
    summary: 'Хулоса',
    overview: 'Умумий кўриниш',
    netBalance: 'Соф баланс',
    totalAccounts: 'Жами ҳисоблар',
    healthStatus: 'Соғломлик ҳолати',
    realTime: 'Реал вақт',
    updated: 'Янгиланган',
    financialHealth: 'Молиявий соғломлик',
    health: {
      excellent: 'Ало',
      good: 'Яхши',
      warning: 'Огоҳлантириш',
      critical: 'Критик',
      unknown: 'Номаълум',
      excellentDescription: 'Барча ҳисоблар мукаммал ҳолатда',
      goodDescription: 'Ҳисоблар яхши ҳолатда',
      warningDescription: 'Баъзи ҳисоблар эътибор талаб қилади',
      criticalDescription: 'Зудлик билан чоралар кўриш керак',
      unknownDescription: 'Ҳолат номаълум'
    }
  },
  budgetActions: {
    title: 'Тезкор ҳаракатлар',
    subtitle: 'Бюджет билан боғлиқ асосий амалларни бажариш',
    convert: {
      description: 'Валюталар ўртасида айирбошлаш'
    },
    refresh: {
      description: 'Бюджет маълумотларини янгилаш'
    }
  },
  budgetModals: {
    convert: {
      subtitle: 'Валюталар ўртасида айирбошлаш',
      swap: 'Валюталарни алмаштириш'
    },
    add: {
      hint: 'Балансга қўшиладиган миқдорни киритинг'
    },
    send: {
      subtitle: 'Ҳисоблар ўртасида пул ўтказиш'
    },
    minus: {
      insufficientFunds: 'Бу операция учун маблағ етарли эмас',
      remainingBalance: 'Қолган баланс'
    }
  },
  financial: {
    clear: 'Тоза',
    hasDebts: 'Қарзлари бор',
    owesUs: 'Бизга қарздор',
    mixed: 'Аралаш'
  },
  // Билдиришнома хабарлари
  notifications: {
    convertSuccess: 'Валюта айирбошлаш муваффақиятли якунланди!',
    convertError: 'Валюта айирбошлашда хатолик юз берди',
    addSuccess: 'Миқдор балансга муваффақиятли қўшилди!',
    addError: 'Миқдор қўшишда хатолик юз берди',
    sendSuccess: 'Ўтказма муваффақиятли якунланди!',
    sendError: 'Ўтказма вақтида хатолик юз берди',
    minusSuccess: 'Миқдор муваффақиятли ечилди!',
    minusError: 'Миқдор ечишда хатолик юз берди'
  },
  // Wallet-га оид таржималар
  wallets: {
    som: {
      title: 'СЎМ Савдо Хамёни',
      description: 'Миллий валюта савдо ҳисоби',
      statsDescription: 'СЎМ савдо ва тижорат операциялари'
    },
    usd: {
      title: 'USD Савдо Хамёни',
      description: 'USD савдо ва тижорат ҳисоби',
      statsDescription: 'USD савдо ва тижорат операциялари',
      sendTooltip: 'Асосий USD га ўтказиш (Корхона раҳбари)',
      sendModalDescription: 'Корхона раҳбарининг асосий USD ҳисобига пул ўтказиш'
    },
    mainUsd: {
      title: 'Асосий USD Хамёни',
      description: 'Корхона раҳбарининг асосий ҳисоби',
      statsDescription: 'Корхона раҳбарининг асосий USD ҳисоби'
    },
    general: {
      payTooltip: 'Хамёндан миқдор ечиш'
    }
  },
  // Қўшимча бюджет атамалар
  currentBalance: 'Жорий баланс',
  addAmount: 'Миқдор қўшиш',
  deductAmount: 'Миқдор ечиш',
  newBalance: 'Янги баланс',
  addingAmount: 'Қўшиладиган миқдор',
  preview: 'Кўриш',
  reason: 'Сабаб',
  warning: 'Огоҳлантириш',
  refresh: 'Янгилаш',
  cancel: 'Бекор қилиш',
  validation: {
    required: 'Бу майдон тўлдирилиши шарт'
  },
  menus: {
    sideBar: {
      budget: 'Бюджет',
      customers: 'Мижозлар',
      cutting: 'Бичув',
      embroidery: 'Вышивка',
      package: 'Упаковка',
      paint: 'Бўёқ',
      productWarehouse: 'Маҳсулот омбори',
      settings: 'Настройка',
      sewing: 'Тикув',
      statistics: 'Статистика',
      sales: 'Савдо',
      orders: 'Буюртмалар',
      users: 'Фойдаланувчилар',
      weaving: 'Тўкув',
      warehouse: 'Oмбор',
      expenses: 'Харажатлар',
      salary:'Маош'
    }
  },
  dialogs: {
    delete: {
      bar: 'Ўчиришни тасдиқлаш',
      info: 'Сиз ушбу элементни ўчиришни истайсизми? Бу амални ортга кайтариб бўлмайди.',
      buttons: {
        confirm: 'Тасдиқлаш',
        cancel: 'Бекор қилиш'
      }
    },
    accept: {
      bar: 'Қабул қилишни тасдиқлаш',
      info: 'Сиз қабул қилишни истайсизми? Бу амални ортга кайтариб бўлмайди.',
      buttons: {
        accept: 'Қабул қилиш',
        cancel: 'Бекор қилиш'
      }
    },
    attendance: {
      bar: 'Ҳозирликни тасдиқлаш',
      info: 'Сиз ҳозирликни қабул қилишни истайсизми? Бу амални ортга кайтариб бўлмайди.',
      buttons: {
        accept: 'Қабул қилиш',
        cancel: 'Бекор қилиш'
      }
    },
    departure: {
      bar: 'Эрта кетишни яратиш',
      buttons: {
        confirm: 'Тасдиқлаш',
      }
    },
    salary: {
      bar: 'Моаш тўлаш'
    },
    warehouse: {
      barUpdate: 'Омборни янгилаш'
    },
    advance: {
      bar: 'Аванс бериш'
    },
    complete: {
      bar: 'Буюртмани тугатиш',
      info: 'Сиз буюртмани тугатишни истайсизми? Бу амални ортга кайтариб бўлмайди.',
      buttons: {
        complete: 'Якунлаш',
        cancel: 'Бекор қилиш'
      }
    },
    reject: {
      bar: 'Рад этишни тасдиқлаш',
      info: 'Сиз рад этишни истайсизми? Бу амални ортга кайтариб бўлмайди.',
      buttons: {
        reject: 'Рад этиш',
        cancel: 'Бекор қилиш'
      }
    },
    user: {
      barCreate: 'Фойдаланувчи яратиш',
      barEdit: 'Фойдаланувчини таҳрирлаш',
    },
    thread: {
      barCreate: 'Ип яратиш',
      barEdit: 'Ипни таҳрирлаш',
    },
    threadPurchase: {
      barCreate: 'Ип сотиб олиш',
      barPayDebt: 'Ип савдосини қарзини тўлаш'
    },
    accessoryPurchase: {
      barCreate: 'Аксессуар сотиб олиш',
      barPayDebt: 'Аксессуар савдосининг қарзини тўлаш'
    },
    indebtedness: {
      barCreate: 'Қарз Олиш',
      barPayDebt: 'Қарзни тўлаш'
    },
    unripeMaterial: {
      barCreate: 'Тўқув материал яратиш',
      barEdit: 'Тўқув материални таҳрирлаш',
    },
    accessory: {
      barCreate: 'Аксессуар яратиш',
      barEdit: 'Аксессуарни таҳрирлаш',
    },
    embroidery: {
      barCreate: 'Вишивка яратиш',
      barEdit: 'Вишивкани таҳрирлаш',
    },
    customer: {
      barCreate: 'Мижоз яратиш',
      barEdit: 'Мижозни таҳрирлаш',
    },
    sale: {
      barCreate: 'Сотув битимини яратиш',
      barReceiveDebt: 'Маҳсулот савдосининг қарзини қабул қилиш'
    },
    unripeMaterialOrder: {
      barCreate: 'Тўқув материал буюртмасини яратиш',
      barEdit: 'Тўқув материал буюртмасини таҳрирлаш',
    },
    modelOrder: {
      barCreate: 'Модел буюртмасини яратиш',
      barEdit: 'Модел буюртмасини таҳрирлаш'
    },
    ripeMaterial: {
      barCreate: 'Материал яратиш',
      barSend: 'Материални юбораш',
      barEdit: 'Материални таҳрирлаш',
    },
    ripeMaterialAccepted: {
      barAccept: 'Материални қабул қилиш',
    },
    ripeMaterialPurchase: {
      barCreate: 'Материал сотиб олиш',
      barPayDebt: 'Материал савдосининг қарзини тўлаш'
    },
    paint: {
      barCreate: 'Бўёқ буюртмасини яратиш',
      barEdit: 'Бўёқ буюртмасини таҳрирлаш',
    },
    repaint: {
      barCreate: 'Қайта бўёқ буюртмаси яратиш',
      barAccept: 'Қабул қилиш',
    },
    paintPurchase: {
      barPayDebt: 'Бўёқ савдосининг қарзини тўлаш'
    },
    completedMaterialOrderReport: {
      barCreate: 'Буюртма ҳақида ҳисобот',
    },
    fabric: {
      barCreate: 'Бўёқ фабрика яратиш',
      barEdit: 'Бўёқ фабрикасини таҳрирлаш',
    },
    color: {
      barCreate: 'Ранг яратиш',
      barEdit: 'Рангни таҳрирлаш',
    },
    model: {
      barCreate: 'Модел яратиш',
      barEdit: 'Моделни таҳрирлаш'
    },
  },
  forms: {
    user: {
      fields: {
        fullName: {
          label: 'Исм ва Фамилия',
          validation: {
            required: 'Исм ва Фамилия талаб қилинади.'
          }
        },
        phone: {
          label: 'Телефон рақам',
          validation: {
            required: 'Телефон рақам талаб қилинади.'
          }
        },
        password: {
          label: 'Парол',
          validation: {
            required: 'Парол талаб қилинади.'
          }
        },
        currency: {
          label: 'Валюта',
          validation: {
            required: 'Валюта талаб қилинади.'
          }
        },
        salary: {
          label: 'Маош',
          validation: {
            required: 'Маош талаб қилинади.'
          }
        },
        roles: {
          label: 'Ҳуҳуқ',
          validation: {
            required: 'Ҳуҳуқ талаб қилинади.',
          }
        },
      },
      buttons: {
        create: 'Фойдаланувчи яратиш',
        edit: 'Фойдаланувчини таҳрирлаш'
      },
      confirmation: {
        successCreated: 'Фойдаланувчи муваффақиятли яратилди.',
        successEdited: 'Фойдаланувчи муваффақиятли таҳрирланди.',
        successDeleted: 'Фойдаланувчи муваффақиятли ўчирилди.',
        failure: 'Фойдаланувчи сақлашда хатолик юз берди.'
      }
    },
    salary: {
      fields: {
        quantity: {
          label: 'Миқдор',
          validation: {
            required: 'Миқдор талаб қилинади.',
          }
        },
      },
      buttons: {
        pay: 'Тўлаш'
      },
      confirmation: {
        successCreated: '',
        successEdited: '',
        successDeleted: '',
        failure: ''
      }
    },
    budget: {
      fields: {
        reason: {
          label: 'Расход сабаби',
          validation: {
            required: 'Сабаб талаб қилинади.',
          }
        },
      }
    },
    warehouse: {
      fields: {
        quantity: {
          label: 'Миқдор',
          validation: {
            required: 'Миқдор талаб қилинади.',
          }
        },
      },
      buttons: {
        update: 'Таҳрирлаш'
      },
      confirmation: {
        successUpdated: 'Омбор муваффақиятли таҳрирланди.',
        failure: 'Хатолик юз берди.'
      }
    },
    advance: {
      fields: {
        quantity: {
          label: 'Миқдор',
          validation: {
            required: 'Миқдор талаб қилинади.',
          }
        },
      },
      buttons: {
        pay: 'Тўлаш'
      },
      confirmation: {
        failure: 'Хатолик юз берди.'
      }
    },
    departure: {
      fields: {
        cutMoney: {
          label: 'Ундурилаётган сумма',
          validation: {
            required: 'Сумма талаб қилинади',
          }
        }
      },
      buttons: {
        create: 'Яратиш',
        edit: 'Таҳрирлаш'
      },
      confirmation: {
        failure: 'Хатолик юз берди'
      }
    },
    thread: {
      fields: {
        name: {
          label: 'Ном',
          validation: {
            required: 'Ном талаб қилинади.'
          }
        },
        measurement: {
          label: 'Ўлчов бирлик',
          validation: {
            required: 'Ўлчов бирлик талаб қилинади.'
          }
        },
        quantity: {
          label: 'Миқдор',
          validation: {
            required: 'Миқдор талаб қилинади.'
          }
        },
        budget: {
          label: 'Бюджетни танланг',
          validation: {
            required: 'Бюджет талаб қилинади.'
          }
        },
        price: {
          label: 'Нарҳ',
          validation: {
            required: 'Нарҳ талаб қилинади.'
          }
        }
      },
      buttons: {
        create: 'Ип яратиш',
        edit: 'Ипни таҳрирлаш'
      },
      confirmation: {
        successCreated: 'Ип муваффақиятли яратилди.',
        successEdited: 'Ип муваффақиятли таҳрирланди.',
        successDeleted: 'Ип муваффақиятли ўчирилди.',
        failure: 'Хатолик юз берди.'
      }
    },
    threadPurchase: {
      fields: {
        thread: {
          label: 'Ном',
          validation: {
            required: 'Ном талаб қилинади.'
          }
        },
        budget: {
          label: 'Бюджетни танланг',
          validation: {
            required: 'Бюджет талаб қилинади.'
          }
        },
        price: {
          label: '1кг нархи?',
          validation: {
            required: 'Нарҳ талаб қилинади.',
          }
        },
        quantity: {
          label: 'Неча кг?',
          validation: {
            required: 'Миқдор талаб қилинади.',
          }
        },
        debtQuantity: {
          label: 'Тўлов миқдори',
          validation: {
            required: 'Миқдор талаб қилинади.',
          }
        },
        paidPrice: {
          label: 'Тўланган сумма',
          validation: {
            required: 'Сумма талаб қилинади.',
          }
        },
      },
      buttons: {
        buy: 'Сотиб олиш',
        payDebt: 'Қарзни тўлаш'
      },
      confirmation: {
        successBought: 'Ип муваффақиятли сотиб олинди.',
        failure: 'Хатолик юз берди.'
      }
    },
    accessoryPurchase: {
      fields: {
        accessory: {
          label: 'Аксессуар номи',
          validation: {
            required: 'Аксессуар номи талаб қилинади.'
          }
        },
        budget: {
          label: 'Қайси бюджетдан?',
          validation: {
            required: 'Бюджет номи талаб қилинади.'
          }
        },
        price: {
          label: '1кг нархи?',
          validation: {
            required: 'Нарҳ талаб қилинади.'
          }
        },
        quantity: {
          label: 'Нечта кг?',
          validation: {
            required: 'Миқдор талаб қилинади.'
          }
        },
        debtQuantity: {
          label: 'Тўлов миқдори',
          validation: {
            required: 'Тўлов миқдори талаб қилинади.'
          }
        },
        paidPrice: {
          label: 'Тўланган сумма',
          validation: {
            required: 'Тўланган сумма талаб қилинади.'
          }
        },
      },
      buttons: {
        buy: 'Сотиб олиш',
        payDebt: 'Қарзни тўлаш'
      },
      confirmation: {
        successBought: 'Аксессуар муваффақиятли сотиб олинди.',
        failure: 'Аксессуар сотиб олишда хатолик юз берди.'
      }
    },
    indebtedness: {
      fields: {
        name: {
          label: 'Ном',
          validation: {
            required: 'Ном талаб қилинади.'
          }
        },
        description: {
          label: 'Таснифи',
          validation: {
            required: 'Тасниф мажбурий.',
          }
        },
        budget: {
          label: 'Бюджет',
          validation: {
            required: 'Бюджет талаб қилинади.'
          }
        },
        quantity: {
          label: 'Миқдор',
          validation: {
            required: 'Миқдор талаб қилинади.'
          }
        },
        price: {
          label: 'Нарҳ',
          validation: {
            required: 'Нарҳ талаб қилинади.'
          }
        },
        dealDate: {
          label: 'Келишилган муддат',
          validation: {
            required: 'Санани киритиш мажбурий.',
          }
        },
      },
      buttons: {
        borrow: 'Қарз олиш',
        payIndebtedness: 'Қарзни тўлаш',
      },
      confirmation: {
        successBought: 'Successfully borrowed',
        failure: 'There was an error borrowing.'
      }
    },
    unripeMaterial: {
      fields: {
        name: {
          label: 'Ном',
          validation: {
            required: 'Ном талаб қилинади.'
          }
        },
        payWorker: {
          label: 'Ишчи нархи',
          validation: {
            required: 'Нарҳ талаб қилинади.'
          }
        },
        payWorkerSort2: {
          label: 'Сорт 2 ишчи нархи',
          validation: {
            required: 'Нарҳ талаб қилинади.'
          }
        },
        budget: {
          label: 'Бюджет',
          validation: {
            required: 'Бюджет талаб қилинади.'
          }
        },
        measurement: {
          label: 'Ўлчов бирлик',
          validation: {
            required: 'Ўлчов бирлик талаб қилинади.'
          }
        },
        quantity: {
          label: 'Миқдор',
          validation: {
            required: 'Миқдор талаб қилинади.'
          }
        },
        quantitySort2: {
          label: 'Миқдор сорт 2',
          validation: {
            required: 'Миқдор талаб қилинади.'
          }
        },
        roll: {
          label: 'Рўлон',
          validation: {
            required: 'Рўлон талаб қилинади.'
          }
        },
        rollSort2: {
          label: 'Рўлон сорт 2',
          validation: {
            required: 'Рўлон талаб қилинади.'
          }
        },
        price: {
          label: 'Нарҳ',
          validation: {
            required: 'Нарҳ талаб қилинади.'
          }
        }
      },
      buttons: {
        create: 'Материал яратиш',
        edit: 'Материални таҳрирлаш'
      },
      confirmation: {
        successCreated: 'Материал муваффақиятли яратилди.',
        successEdited: 'Материал муваффақиятли таҳрирланди.',
        successDeleted: 'Материал муваффақиятли ўчирилди.',
        failure: 'Материал сақлашда хатолик юз берди.'
      }
    },
    accessory: {
      fields: {
        name: {
          label: 'Ном',
          validation: {
            required: 'Ном талаб қилинади.'
          }
        },
        image: {
          label: 'Расм',
          validation: {
            required: 'Расм талаб қилинади.'
          }
        },
        type: {
          label: 'Турини танланг',
          validation: {
            required: 'Тур талаб қилинади.',
          }
        },
        measurement: {
          label: 'Ўлчов бирлик',
          validation: {
            required: 'Ўлчов бирлик талаб қилинади.'
          }
        },
        quantity: {
          label: 'Миқдор',
          validation: {
            required: 'Миқдор талаб қилинади.'
          }
        },
        budget: {
          label: 'Бюджет',
          validation: {
            required: 'Бюджет талаб қилинади.'
          }
        },
        price: {
          label: 'Нарҳ',
          validation: {
            required: 'Нарҳ талаб қилинади.'
          }
        }
      },
      buttons: {
        create: 'Аксессуар яратиш',
        edit: 'Аксессуарни таҳрирлаш'
      },
      confirmation: {
        successCreated: 'Аксессуар муваффақиятли яратилди.',
        successEdited: 'Аксессуар муваффақиятли таҳрирланди.',
        successDeleted: 'Аксессуар муваффақиятли ўчирилди.',
        failure: 'Аксессуар сақлашда хатолик юз берди.'
      }
    },
    embroidery: {
      fields: {
        name: {
          label: 'Ном',
          validation: {
            required: 'Ном талаб қилинади.'
          }
        },
        image: {
          label: 'Расм',
          validation: {
            required: 'Расм талаб қилинади.'
          }
        },
        workerPrice: {
          label: 'Ишчи нархи',
          validation: {
            required: 'Нарҳ талаб қилинади.'
          }
        },
        budget: {
          label: 'Бюджет',
          validation: {
            required: 'Бюджет талаб қилинади.'
          }
        }
      },
      buttons: {
        create: 'Вишивка яратиш',
        edit: 'Вишивкани таҳрирлаш'
      },
      confirmation: {
        successCreated: 'Вишивка муваффақиятли яратилди.',
        successEdited: 'Вишивка муваффақиятли таҳрирланди.',
        successDeleted: 'Вишивка муваффақиятли ўчирилди.',
        failure: 'Вишивкани сақлашда хатолик юз берди.'
      }
    },
    customer: {
      fields: {
        fullName: {
          label: 'Исм ва Фамилия',
          validation: {
            required: 'Исм ва Фамилия талаб қилинади.'
          }
        },
        username: {
          label: 'Фойдаланувчи номи',
          validation: {
            required: 'Фойдаланувчи номи талаб қилинади.'
          }
        },
        phone: {
          label: 'Телефон рақам',
          validation: {
            required: 'Телефон рақам талаб қилинади.'
          }
        }
      },
      buttons: {
        create: 'Мижоз яратиш',
        edit: 'Мижозни таҳрирлаш'
      },
      confirmation: {
        successCreated: 'Мижоз муваффақиятли яратилди.',
        successEdited: 'Мижоз муваффақиятли таҳрирланди.',
        successDeleted: 'Мижоз муваффақиятли ўчирилди.',
        failure: 'Мижозни сақлашда хатолик юз берди.'
      }
    },
    sale: {
      fields: {
        customer: {
          label: 'Мижозни танланг',
          validation: {
            required: 'Мижоз талаб қилинади.'
          }
        },
        budget: {
          label: 'Бюджет',
          validation: {
            required: 'Бюджет талаб қилинади.'
          }
        },
        productModel: {
          label: 'Модель',
          validation: {
            required: 'Модель талаб қилинади.'
          }
        },
        size: {
          label: 'Размер',
          validation: {
            required: 'Размер талаб қилинади.'
          }
        },
        quantity: {
          label: 'Миқдор',
          validation: {
            required: 'Миқдор талаб қилинади.'
          }
        },
        price: {
          label: 'Нарҳ',
          validation: {
            required: 'Нарҳ талаб қилинади.'
          }
        },
        paidPrice: {
          label: 'Тўланган сумма',
          validation: {
            required: 'Тўланган сумма талаб қилинади.'
          }
        },
        debtQuantity: {
          label: 'Тўлов миқдори',
          validation: {
            required: 'Тўлов миқдори талаб қилинади.',
          }
        },
        subtotal: {
          label: 'Жами (чегирмасиз)'
        },
        discount: {
          label: 'Чегирма',
          hint: 'Чегирма миқдорини киритинг (агар бор бўлса)',
          validation: {
            required: 'Чегирма 0 дан катта ва умумий нархдан кам бўлиши керак.'
          }
        },
        finalPrice: {
          label: 'Тўлов суммаси'
        },
        additionalDiscount: {
          label: 'Қўшимча чегирма',
          hint: 'Қарз тўлашда қўшимча чегирма бериш (ихтиёрий)',
          validation: {
            required: 'Чегирма 0 дан катта ва қолган қарздан кам бўлиши керак.'
          }
        },
        finalPaymentAmount: {
          label: 'Тўланадиган сумма'
        },
        paymentAmountHint: 'Тўлаш учун миқдорни киритинг'
      },
      sections: {
        basicInfo: 'Асосий маълумот',
        products: 'Маҳсулотлар',
        paymentDetails: 'Тўлов маълумотлари',
        addProduct: 'Маҳсулот қўшиш',
        product: 'Маҳсулот',
        deleteProduct: 'Ўчириш',
        remainingDebt: 'Қолган қарз',
        pleaseSelectCustomerBudget: 'Илтимос, аввал мижоз ва бюджетни танланг'
      },
      buttons: {
        create: 'Савдо',
        receiveDebt: 'Қарзни олиш'
      },
      confirmation: {
        successCreated: 'Савдо муваффақиятли яратилди.',
        successEdited: 'Савдо муваффақиятли таҳрирланди.',
        successDeleted: 'Савдо муваффақиятли ўчирилди.',
        failure: 'Савдони сақлашда хатолик юз берди.'
      }
    },
    ripeMaterial: {
      fields: {
        name: {
          label: 'Ном',
          validation: {
            required: 'Ном талаб қилинади.'
          }
        },
        color: {
          label: 'Ранг',
          validation: {
            required: 'Ранг талаб қилинади.'
          }
        },
        measurement: {
          label: 'Ўлчов бирлик',
          validation: {
            required: 'Ўлчов бирлик талаб қилинади.'
          }
        },
        quantity: {
          label: 'Миқдор',
          validation: {
            required: 'Миқдор талаб қилинади.'
          }
        },
        roll: {
          label: 'Ролон',
          validation: {
            required: 'Ролон талаб қилинади.'
          }
        },
        quantitySort2: {
          label: 'Миқдор 2-сорт',
          validation: {
            required: 'Миқдор талаб қилинади.'
          }
        },
        rollSort2: {
          label: 'Ролон 2-сорт',
          validation: {
            required: 'Ролон талаб қилинади.'
          }
        },
        budget: {
          label: 'Бюджетни танланг',
          validation: {
            required: 'Бюджет талаб қилинади.'
          }
        },
        price: {
          label: 'Нарҳ',
          validation: {
            required: 'Нарҳ талаб қилинади.'
          }
        },
        priceSort2: {
          label: 'Нарҳ 2-сорт',
          validation: {
            required: 'Нарҳ талаб қилинади.'
          }
        }
      },
      buttons: {
        create: 'Яратиш',
        edit: 'Таҳрирлаш'
      },
      confirmation: {
        successCreated: 'Материал муваффақиятли яратилди.',
        successEdited: 'Материал муваффақиятли таҳрирланди.',
        successDeleted: 'Материал муваффақиятли ўчирилди.',
        failure: 'Материални сақлашда хатолик юз берди.'
      }
    },
    ripeMaterialAccepted: {
      fields: {
        ripeMaterial: {
          label: 'Материални танланг',
          validation: {
            required: 'Материал талаб қилинади.',
          }
        },
        receivedQuantitySort1: {
          label: 'Миқдор Сорт 1',
          validation: {
            required: 'Миқдор мажбурий.',
          }
        },
        receivedRollSort1: {
          label: 'Ролон Сорт 1',
          validation: {
            required: 'Ролон талаб қилинади.',
          }
        },
        receivedQuantitySort2: {
          label: 'Миқдор Сорт 2',
          validation: {
            required: 'Миқдор талаб қилинади.',
          }
        },
        receivedRollSort2: {
          label: 'Ролон Сорт 2',
          validation: {
            required: 'Ролон талаб қилинади.',
          }
        },
        budget: {
          label: 'Бюджетни танланг',
          validation: {
            required: 'Бюджет мажбурий.',
          }
        },
        servicePrice: {
          label: 'Хизмат нархи',
          validation: {
            required: 'Хизмат нархи талаб қилинади.',
          }
        },
        price: {
          label: '1(кг/м) учун нарҳ?',
          validation: {
            required: 'Нарҳ мажбурий.',
          }
        },
        payedPrice: {
          label: 'Тўланган нарҳ',
          validation: {
            required: 'Нарҳ талаб қилинади.',
          }
        },
      },
      buttons: {
        accept: 'Қабул қилиш',
      },
      confirmation: {
        successAccepted: 'Материал муваффақиятли қабул қилинди.',
        failure: 'Материални сақлашда хатолик юз берди.',
      }
    },
    paint: {
      fields: {
        unripeMaterial: {
          label: 'Тоқув материал',
          validation: {
            required: 'Ном талаб қилинади.',
          }
        },
        paintFabric: {
          label: 'Фабрикани танланг',
          validation: {
            required: 'Фабрика талаб қилинади.',
          }
        },
        sentQuantitySort1: {
          label: 'Миқдор',
          validation: {
            required: 'Миқдор талаб қилинади.',
          }
        },
        sentQuantitySort2: {
          label: 'Миқдор',
          validation: {
            required: 'Миқдор талаб қилинади.',
          }
        },
        sentRollSort1: {
          label: 'Ролон',
          validation: {
            required: 'Ролон талаб қилинади.',
          }
        },
        sentRollSort2: {
          label: 'Ролл',
          validation: {
            required: 'Ролон талаб қилинади.',
          }
        },
        dealDate: {
          label: 'Келишилган муддат',
          validation: {
            required: 'Санани киритиш мажбурий.',
          }
        },
      },
      buttons: {
        order: 'Буйрутма яратиш',
        edit: 'Таҳрирлаш',
      },
      confirmation: {
        successOrderCreated: 'Буйрутма муваффақиятли яратилди.',
        successOrderReceived: 'Буйрутма муваффақиятли қабул қилинди.',
        successOrderCompleted: 'Буйрутма муваффақиятли якунланди.',
        successOrderEdited: 'Буйрутма муваффақиятли таҳрирланди.',
        successOrderDeleted: 'Буйрутма муваффақиятли ўчирилди.',
        failure: 'Буйрутмани сақлашда хатолик юз берди.',
      }
    },
    repaint: {
      fields: {
        ripeMaterial: {
          label: 'Материални танланг',
          validation: {
            required: 'Материал мажбурий.',
          }
        },
        paintFabric: {
          label: 'Фабрикани танланг',
          validation: {
            required: 'Фабрика мажбурий.',
          }
        },
        sentQuantitySort1: {
          label: 'Юборилаётган миқдор сорт 1',
          validation: {
            required: 'Миқдор мажбурий.',
          }
        },
        receivedQuantitySort1: {
          label: 'Қабул қилинган миқдор сорт 1',
          validation: {
            required: 'Миқдор мажбурий.',
          }
        },
        sentQuantitySort2: {
          label: 'Юборилаётган миқдор сорт 2',
          validation: {
            required: 'Миқдор мажбурий.',
          }
        },
        receivedQuantitySort2: {
          label: 'Қабул қилинган миқдор сорт 2',
          validation: {
            required: 'Миқдор мажбурий.',
          }
        },
        sentRollSort1: {
          label: 'Юборилаётган ролон Сорт 1',
          validation: {
            required: 'Ролон мажбурий.',
          }
        },
        receivedRollSort1: {
          label: 'Қабул қилинган ролон сорт 1',
          validation: {
            required: 'Ролон мажбурий.',
          }
        },
        sentRollSort2: {
          label: 'Юборилаётган ролон сорт 2',
          validation: {
            required: 'Ролон мажбурий.',
          }
        },
        receivedRollSort2: {
          label: 'Қабул қилинган ролон сорт 2',
          validation: {
            required: 'Ролон мажбурий.',
          }
        },
      },
      buttons: {
        order: 'Буйрутма яратиш',
        accept: 'Буйрутмани қабул қилиш',
      },
      confirmation: {
        successOrderCreated: 'Буйрутма муваффақиятли яратилди.',
        successOrderReceived: 'Буйрутма муваффақиятли қабул қилинди.',
        failure: 'Буйрутмани сақлашда хатолик юз берди.',
      }
    },
    ripeMaterialPurchase: {
      fields: {
        ripeMaterial: {
          label: 'Материал номи',
          validation: {
            required: 'Ном мажбурий.',
          }
        },
        budget: {
          label: 'Қайси Бюджетдан?',
          validation: {
            required: 'Бюджет мажбурий.',
          }
        },
        price: {
          label: '1 (кг/м) нархи?',
          validation: {
            required: 'Нарх мажбурий.',
          }
        },
        priceSort2: {
          label: '1 (кг/м) нархи сорт 2?',
          validation: {
            required: 'Нарх мажбурий.',
          }
        },
        roll: {
          label: 'Неча ролон?',
          validation: {
            required: 'Ролон мажбурий.',
          }
        },
        quantity: {
          label: 'Неча (кг/м)?',
          validation: {
            required: 'Миқдор мажбурий.',
          }
        },
        quantitySort2: {
          label: 'Неча (кг/м) сорт-2?',
          validation: {
            required: 'Миқдор мажбурий.',
          }
        },
        rollSort2: {
          label: 'Нечта ролон сорт-2?',
          validation: {
            required: 'Ролон мажбурий.',
          }
        },
        debtQuantity: {
          label: 'Тўлов миқдори',
          validation: {
            required: 'Тўлов миқдор мажбурий.',
          }
        },
        paidPrice: {
          label: 'Тўланган сумма',
          validation: {
            required: 'Сумма мажбурий.',
          }
        },
      },
      buttons: {
        buy: 'Сотиб олиш',
        payDebt: 'Тўлов қилиш',
        send: 'Юбориш'
      },
      confirmation: {
        successBought: 'Материал муваффақиятли сотиб олинди.',
        successSent: 'Материал муваффақиятли юбораилди.',
        failure: 'Материални сотиб олишда хатолик юз берди.',
        failureSent: 'Материални юбораишда хатолик юз берди.'
      }
    },
    unripeMaterialOrder: {
      fields: {
        quantity: {
          label: 'Миқдор',
          validation: {
            required: 'Миқдор керак.',
          }
        },
        material: {
          label: 'Материал',
          validation: {
            required: 'Материал мажбурий.',
          }
        },
        expectedConsumeDtos: {
          title: 'Кутилган сарф:',
          thread: {
            label: 'Ип тури',
            validation: {
              required: 'Ип тури мажбурий.',
            }
          },
          quantity: {
            label: 'Миқдор',
            validation: {
              required: 'Миқдор мажбурий.',
            }
          }
        }
      },
      buttons: {
        create: 'Буюртма яратиш',
        edit: 'Буюртмани таҳрирлаш'
      },
      confirmation: {
        successCreated: 'Тоқув материал буюртмаси муваффақиятли яратилди.',
        successCompleted: 'Тоқув материал буюртмаси муваффақиятли бажарилди.',
        successDeleted: 'Тоқув материал буюртмаси муваффақиятли ўчирилди.',
        failure: 'Тоқув материал буюртмаси сақлашда хатолик юз берди.'
      }
    },
    modelOrder: {
      fields: {
        productModel: {
          label: 'Моделни танланг',
          validation: {
            required: 'Модел номи мажбурий.',
          }
        },
        expectedOutlayRipeMaterial: {
          title: 'Кутилган сарф:',
        },
        cutterRipeMaterialWarehouse: {
          label: 'Материални танланг',
          validation: {
            required: 'Материал номи мажбурий.',
          }
        },
        size: {
          label: 'Модел размер',
          validation: {
            required: 'Размер мажбурий.',
          }
        },
        quantity: {
          label: 'Модел миқдори',
          validation: {
            required: 'Модел миқдори мажбурий.',
          }
        },
        expectedConsumeQuantity: {
          label: 'Кутилган сарф миқдори',
          validation: {
            required: 'Миқдор мажбурий.',
          }
        },
        consumedQuantity: {
          label: 'Маҳсулотлар учун ишлатилган миқдор (кг/м)?',
          validation: {
            required: 'Миқдор мажбурий.',
          }
        },
        wasteSort1: {
          label: 'Неча (кг/м) чиқиндий?',
          validation: {
            required: 'Миқдор мажбурий.',
          }
        },
        remainingSort1: {
          label: 'Неча (кг/м) қолдиқ ишлатилган?',
          validation: {
            required: 'Миқдор мажбурий.',
          }
        },
        wasteRemainingSort1: {
          label: 'Неча (кг/м) қолдиқнинг чиқиндиси?',
          validation: {
            required: 'Миқдор мажбурий.',
          }
        },
        newRemainingSort1: {
          label: 'Неча (кг/м) янги қолдиқ?',
          validation: {
            required: 'Миқдор мажбурий.',
          }
        },
        expectedConsumeQuantitySort2: {
          label: 'Кутилган сарф миқдори сорт 2',
          validation: {
            required: 'Миқдор мажбурий.',
          }
        },
        quantitySort2: {
          label: 'Маҳсулотлар учун ишлатилган сорт 2 миқдор (кг/м)?',
          validation: {
            required: 'Миқдор мажбурий.',
          }
        },
        wasteSort2: {
          label: 'Неча (кг/м) сорт 2 чиқиндий?',
          validation: {
            required: 'Миқдор мажбурий.',
          }
        },
        remainingSort2: {
          label: 'Неча (кг/м) сорт 2 қолдиқ ишлатилган?',
          validation: {
            required: 'Миқдор мажбурий.',
          }
        },
        wasteRemainingSort2: {
          label: 'Неча (кг/м) сорт 2 қолдиқнинг чиқиндиси?',
          validation: {
            required: 'Миқдор мажбурий.',
          }
        },
        newRemainingSort2: {
          label: 'Неча (кг/м) сорт 2 янги қолдиқ?',
          validation: {
            required: 'Миқдор мажбурий.',
          }
        },
        expectedConsumeRemainingSort1: {
          label: 'Сорт 1 қолдиқ миқдор',
          validation: {
            required: 'Миқдор талаб қилинди.',
          }
        },
        expectedConsumeRemainingSort2: {
          label: 'Сорт 2 қолдиқ миқдор',
          validation: {
            required: 'Миқдор талаб қилинди.',
          }
        }
      },
      buttons: {
        create: 'Буюртма яратиш',
        edit: 'Буюртмани таҳрирлаш'
      },
      confirmation: {
        successCreated: 'Буюртма муваффақиятли яратилди.',
        successEdited: 'Буюртма муваффақиятли таҳрирланди.',
        successReceived: 'Буюртма муваффақиятли қабул қилинди.',
        successCompleted: 'Буюртма муваффақиятли якунланди.',
        failure: 'Хатолик юз берди',
        failureReceived: 'Буюртмани қабул қилишда хатолик юз берди'
      }
    },
    completedMaterialOrderReport: {
      fields: {
        sort: {
          label: 'Сортни танланг?',
          validation: {
            required: 'Сорт талаб қилинади.',
          }
        },
        roll: {
          label: 'Нечта ролон?',
          validation: {
            required: 'Ролон талаб қилинади.',
          }
        },
        quantity: {
          label: 'Неча кг тайёр?',
          validation: {
            required: 'Миқдор талаб қилинади.',
          }
        },
        consumedDtos: {
          title: 'Ишлатилган маҳсулотлар:',
          thread: {
            label: 'Ип тури',
            validation: {
              required: 'Ип тури мажбурий',
            }
          },
          quantity: {
            label: 'Миқдор',
            validation: {
              required: 'Миқдор мажбурий.',
            }
          }
        },
      },
      buttons: {
        create: 'Ҳисобот'
      },
      confirmation: {
        successCreated: 'Ҳисобот муваффақиятли яратилди.',
        successAccepted: 'Ҳисобот муваффақиятли қабул қилинди.',
        successRejected: 'Ҳисобот рад этилди.',
        failure: 'Хатолик юз берди'
      }
    },
    fabric: {
      fields: {
        name: {
          label: 'Ном',
          validation: {
            required: 'Ном мажбурий.',
          }
        },
        address: {
          label: 'Манзил',
          validation: {
            required: 'Манзил мажбурий.',
          }
        }
      },
      buttons: {
        create: 'Фабрика яратиш',
        edit: 'Фабрикани таҳрирлаш'
      },
      confirmation: {
        successCreated: 'Фабрика муваффақиятли яратилди.',
        successEdited: 'Фабрика муваффақиятли таҳрирланди.',
        successDeleted: 'Фабрика муваффақиятли ўчирилди.',
        failure: 'Хатолик юз берди.'
      }
    },
    color: {
      fields: {
        name: {
          label: 'Ранг номи',
          validation: {
            required: 'Ном мажбурий',
          }
        }
      },
      buttons: {
        create: 'Ранг яратиш',
        edit: 'Ранг таҳрирлаш'
      },
      confirmation: {
        successCreated: 'Ранг муваффақиятли яратилди.',
        successEdited: 'Фабрика муваффақиятли таҳрирланди',
        successDeleted: 'Фабрика муваффақиятли ўчирилди.',
        failure: 'Хатолик юз берди.'
      }
    },
    model: {
      fields: {
        name: {
          label: 'Модел номи',
          validation: {
            required: 'Модел номи мажбурий.',
          }
        },
        description: {
          label: 'Модел таснифи',
          validation: {
            required: 'Тасниф мажбурий.',
          }
        },
        budget: {
          label: 'Бюджетни танланг',
          validation: {
            required: 'Бюджет мажбурий.',
          }
        },
        sizes: {
          title: 'Модел размерларини яратиш:',
        },
        size: {
          label: 'Модел размер номи',
          validation: {
            required: 'Модел размер номи мажбурий.',
          }
        },
        price: {
          label: 'Модел нархи',
          validation: {
            required: 'Модел нархи мажбурий.',
          }
        },
        embroideries: {
          label: 'Вишивкани танланг',
          validation: {
            required: 'Вишивка мажбурий.',
          }
        },
        productAccessories: {
          title: 'Маҳсулот аксесуарларини қўшиш:',
          accessory: {
            label: 'Аксесуарни танлаш',
            validation: {
              required: 'Аксесуар мажбурий.',
            }
          },
          quantity: {
            label: 'Неҷта?',
            validation: {
              required: 'Миқдор мажбурий.',
            }
          },
          budget: {
            label: 'Ишчи нархининг бюджетини танланг',
            validation: {
              required: 'Бюджетини мажбурий.',
            }
          },
          workerPrice: {
            label: 'Ишчи учун бериладиган сумма',
            validation: {
              required: 'Сумма талаб қилинади.',
            }
          }
        }
      },
      buttons: {
        create: 'Модел яратиш',
        edit: 'Модел таҳрирлаш'
      },
      confirmation: {
        successCreated: 'Модел муваффақиятли яратилди.',
        successEdited: 'Модел муваффақиятли таҳрирланди.',
        successDeleted: 'Модел муваффақиятли ўчирилди',
        failure: 'Хатолик юз берди.'
      }
    },
  },
  tables: {
    workEntry: {
      header: {
        title: 'Бажарилган ишлар рўйхат',
        empty: 'Ҳеч қандай ишлар топилмади'
      },
      columns: {
        createdAt: 'Кун',
        workerBy: 'Исм',
        productAccessory: 'Модель номи',
        embroidery: 'Вышивка',
        quantity: 'Миқдор',
        unitPrice: 'Донасининг нархи',
        totalPrice: 'Жами нархи',
        confirmedBy: 'Тасдиқлади',
      },
    },
    exchange: {
      header: {
        title: 'Пул алмашинувлари',
        empty: 'Ҳеч қандай алмашинув топилмади',
      },
      columns: {
        id: 'ID',
        createdAt: 'Сана',
        fromBudget: 'Дан',
        toBudget: 'Га',
        quantity: 'Миқдор',
        description: 'Тасниф',
        status: 'Статус',
        createdBy: 'Яратди',
        confirmedBy: 'Тасдиқлади',
      },
    },
    users: {
      header: {
        title: 'Фойдаланувчилар',
        searchTitle: 'Исм бўйича қидириш',
        empty: 'Ҳеч қандай фойдаланувчи топилмади'
      },
      columns: {
        worker: 'Ишчи',
        month: 'Ой',
        baseSalary: 'Асосий иш ҳаққи',
        dailyWage: 'Кунлик иш ҳаққи',
        workedDays: 'Ишланган кунлар',
        pieceworkEarning: 'Ишбайлик ҳаққи',
        advancePayment: 'Аванс тўлов',
        paidAmount: 'Тўланган сумма',
        transaction: 'Тасниф',
        fullName: 'Исм & Фамилия',
        phone: 'Телефон',
        salary: 'Ойлик',
        salaryType: 'Ойлик тури',
        currency: 'Валютa',
        role: 'Рол'
      },
      buttons: {
        add: 'Фойдаланувчи қўшиш'
      },
    },
    attendance: {
      header: {
        title: 'Давомат',
        empty: 'Ҳеч қандай давомат топилмади'
      },
      columns: {
        worker: 'Ишчи',
        date: 'Кун',
        isWork: 'Ишда',
        cutMoney: 'Ундурилган сумма',
        isTimelyDeparture: 'Вақтдан олдин кетган',
      }
    },
    indebtedness: {
      header: {
        title: 'Қарздорлик',
        empty: 'Ҳеч қандай Қарздорлик топилмади'
      },
      columns: {
        id: 'id',
        createdAt: 'Сана',
        dealDate: 'Келишилган сана',
        name: 'Ном',
        description: 'Тасниф',
        quantity: 'Миқдор',
        remainingAmount: 'Қолган Қарз',
        transaction: 'Тасниф'
      },
      buttons: {
        add: 'Қарз олиш'
      }
    },
    thread: {
      header: {
        title: 'Ип',
        empty: 'Ҳеч қандай ип топилмади'
      },
      columns: {
        name: 'Ном',
        quantity: 'Миқдор',
        price: 'Нарх',
      },
      buttons: {
        add: 'Ип қўшиш',
        buy: 'Сотиб олиш',
      },
    },
    threadPurchase: {
      header: {
        title: 'Ип Савдоси',
        empty: 'Ҳеч қандай ип савдоси топилмади'
      },
      columns: {
        id: 'Савдо ID',
        createdAt: 'Яратилган вақт',
        thread: 'Ип',
        quantity: 'Миқдор',
        price: 'Нарх',
        totalPrice: 'Жами нарх',
        paidPrice: 'Тўланган сумма',
        purchasedBy: 'Ким томонидан савдо қилинган',
        transaction: 'Тасниф',
      },
      buttons: {
        add: 'Ип қўшиш'
      },
    },
    accessoryPurchase: {
      header: {
        title: 'Аксессуар Савдоси',
        empty: 'Ҳеч қандай Аксессуар Савдоси топилмади'
      },
      columns: {
        id: 'Савдо ID',
        createdAt: 'Яратилган вақт',
        accessory: 'Аксессуар',
        quantity: 'Миқдор',
        price: 'Нарх',
        totalPrice: 'Жами нарх',
        paidPrice: 'Тўланган сумма',
        purchasedBy: 'Ким томонидан савдо қилинган',
        transaction: 'Тасниф',
      },
      buttons: {
        add: 'Савдо яратиш'
      },
    },
    transaction: {
      header: {
        title: 'Транзакциялар',
        empty: 'Ҳеч қандай Транзакция топилмади'
      },
      columns: {
        id: 'ID',
        createdAt: 'Сана',
        paidPrice: 'Тўланган сумма',
        createdBy: 'Ким томонидан яратилган',
        description: 'Сабаб',
        budget: 'Бюджет',
        price: 'Нарх'
      },
      buttons: {
        add: 'Транзакция яратиш'
      },
    },
    unripeMaterial: {
      header: {
        title: 'Тўқув Материаллари',
        empty: 'Ҳеч қандай Материал топилмади'
      },
      columns: {
        name: 'Номи',
        quantity: 'Миқдор',
        roll: 'Ролон',
        quantitySort2: 'Миқдор сорт 2',
        rollSort2: 'Ролон сорт 2',
        price: 'Нарх',
        payWorker: 'Ишчи ҳаққи'
      },
      buttons: {
        add: 'Материал қўшиш'
      },
    },
    accessory: {
      header: {
        title: 'Аксессуар',
        empty: 'Ҳеч қандай Аксессуар топилмади'
      },
      columns: {
        name: 'Ном',
        image: 'Расм',
        quantity: 'Миқдор',
        price: 'Нарх',
        type: 'Бўлим',
      },
      buttons: {
        add: 'Аксессуар қўшиш',
        purchase: 'Сотиб олиш'
      },
    },
    embroidery: {
      header: {
        title: 'Вышивка',
        empty: 'Ҳеч қандай Вышивка топилмади'
      },
      columns: {
        name: 'Ном',
        image: 'Расм',
        workerPrice: 'Ишчи ҳаққи',
      },
      buttons: {
        add: 'Вышивка қўшиш',
      },
    },
    customer: {
      header: {
        title: 'Мижоз',
        empty: 'Ҳеч қандай Мижоз топилмади'
      },
      columns: {
        fullName: 'Исм & Фамилия',
        username: 'Фойдаланувчи номи',
        phone: 'Телефон',
        debt: 'Карз',
      },
      buttons: {
        add: 'Мижоз қўшиш',
      },
    },
    sale: {
      header: {
        title: 'Савдо Трансаксиялари',
        empty: 'Ҳеч қандай Савдо Трансаксия топилмади'
      },
      columns: {
        id: 'Савдо ID',
        createdAt: 'Сана',
        purchasedBy: 'Ким',
        purchasedTo: 'Кимга',
        saleProduct: 'Маҳсулотлар',
        totalPrice: 'Жами нарх',
        originalPrice: 'Асосий нарх',
        discount: 'Чегирма',
        finalPrice: 'Тўлов суммаси',
        paidPrice: 'Тўланган сумма',
        debt: 'Карз',
        transaction: 'Тасниф',
      },
      buttons: {
        add: 'Яратиш'
      },
      tooltip: {
        clickToEnablePagination: 'Саҳифалашни ёқиш учун босинг',
        clickToShowAll: 'Барчасини кўрсатиш учун босинг'
      },
      pagination: {
        showing: 'Кўрсатилмоқда',
        of: 'дан',
        showingAllItems: 'Барча элементлар кўрсатилмоқда'
      }
    },
    order: {
      header: {
        title: 'Буюртмалар',
        empty: 'Ҳеч қандай Буюртма топилмади'
      },
      columns: {
        id: 'ID',
        createdAt: 'Сана',
        createdBy: 'Ким',
        customer: 'Кимга',
        products: 'Маҳсулотлар',
        status: 'Статус',
        totalPrice: 'Жами нарх',
        dealDate: 'Келишилган сана',
      },
      buttons: {
        add: 'Қўшиш',
      }
    },
    ripeMaterial: {
      header: {
        title: 'Материаллар',
        empty: 'Ҳеч қандай Материал топилмади'
      },
      columns: {
        name: 'Ном',
        paintFabricColor: 'Ранг',
        quantity: 'Миқдор',
        roll: 'Ролон',
        quantitySort2: 'Миқдор сорт 2',
        rollSort2: 'Ролон сорт 2',
        price: 'Нарх',
        priceSort2: 'Нарх сорт 2'
      },
      buttons: {
        add: 'Қўшиш',
        buy: 'Сотиб олиш',
        sendToCutter: 'Бичувчи омборига юбориш'
      }
    },
    cutterRipeMaterial: {
      header: {
        title: 'Материаллар',
        empty: 'Ҳеч қандай Материал топилмади'
      },
      columns: {
        ripeMaterial: 'Материал номи',
        quantity: 'Миқдор',
        roll: 'Ролон',
        quantitySort2: 'Миқдор сорт 2',
        rollSort2: 'Ролон сорт 2',
        remainingSort1: 'Қолдиқ сорт 1',
        remainingSort2: 'Қолдиқ сорт 2'
      },
      buttons: {
        sendToWarehouse: 'Складга юбориш'
      }
    },
    ripeMaterialPurchase: {
      header: {
        title: 'Материал Савдоси',
        empty: 'Ҳеч қандай савдо топилмади'
      },
      columns: {
        id: 'Савдо ID',
        createdAt: 'Сана',
        ripeMaterial: 'Материал',
        quantity: 'Миқдор',
        quantitySort2: 'Миқдор сорт 2',
        price: 'Нарх',
        priceSort2: 'Нарх сорт 2',
        roll: 'Рўлон',
        rollSort2: 'Рўлон сорт 2',
        paidPrice: 'Тўланган сумма',
        totalPrice: 'Жами нарх',
        purchasedBy: 'Ким томонидан савдо қилинган',
        transaction: 'Тасниф',
      },
      buttons: {
        add: 'Савдо қилиш'
      },
    },
    paint: {
      header: {
        title: 'Бўёқ Буюртмалари',
        empty: 'Ҳеч қандай буюртма топилмади'
      },
      columns: {
        sentQuantitySort1: 'Миқдор сорт 1',
        sentRollSort1: 'Рўлон сорт 1',
        sentQuantitySort2: 'Миқдор сорт 2',
        sentRollSort2: 'Рўлон сорт 2',
        status: 'Статус',
        createdBy: 'Ким томонидан яратилган',
        receivedBy: 'Ким қабул қилган',
        paintFabric: 'Фабрика',
        createdAt: 'Сана',
        receivedAt: 'Қабул вақти',
        dealDate: 'Келишилган сана',
        unripeMaterial: 'Тўқув Материал',
        ripeMaterialOrderAccepteds: 'Тасниф'
      },
      buttons: {
        add: 'Буюртма қўшиш'
      }
    },
    repaint: {
      header: {
        title: 'Қайта Бўёқ Буюртмалари',
        empty: 'Ҳеч қандай Қайта Бўёқ Буюртмалари топилмади'
      },
      columns: {
        createdAt: 'Сана',
        ripeMaterial: 'Материал',
        createdBy: 'Ким томонидан яратилган',
        paintFabric: 'Фабрика',
        sentQuantitySort1: 'Юборилган миқдор сорт 1',
        receivedQuantitySort1: 'Қабул қилинган миқдор сорт 1',
        sentRollSort1: 'Юборилган рўлон сорт 1',
        receivedRollSort1: 'Қабул қилинган рўлон сорт 1',
        sentQuantitySort2: 'Юборилган миқдор сорт 2',
        receivedQuantitySort2: 'Қабул қилинган миқдор сорт 2',
        sentRollSort2: 'Юборилган рўлон сорт 2',
        receivedRollSort2: 'Қабул қилинган рўлон сорт 2',
        status: 'Статус',
        receivedBy: 'Ким қабул қилган',
        receivedAt: 'Қабул вақт',
        dealDate: 'Келишилган сана'
      },
      buttons: {
        add: 'Буюртма яратиш'
      }
    },
    paintPurchase: {
      header: {
        title: 'Бўёқ Савдоси',
        empty: 'Ҳеч қандай савдо топилмади'
      },
      columns: {
        id: 'Савдо ID',
        receivedBy: 'Ким қабул қилган',
        receivedAt: 'Қабул вақт',
        ripeMaterial: 'Материал',
        receivedQuantitySort1: 'Миқдор сорт 1',
        receivedRollSort1: 'Рўлон сорт 1',
        receivedQuantitySort2: 'Миқдор сорт 2',
        receivedRollSort2: 'Рўлон сорт 2',
        servicePrice: 'Сервис нарх',
        payedPrice: 'Тўланган сумма',
        transaction: 'Тасниф',
      }
    },
    unripeMaterialOrder: {
      header: {
        title: 'Тўқув Материал Буюртма',
        empty: 'Ҳеч қандай буюртма топилмади'
      },
      columns: {
        id: 'Буюртма ID',
        material: 'Материал номи',
        quantity: 'Миқдор',
        completedQuantity: 'Тугатилган миқдор',
        completedRoll: 'Рўлон',
        completedQuantitySort2: 'Миқдор сорт 2',
        completedRollSort2: 'Рўлон сорт 2',
        expectedConsume: 'Кутилаётган сарф',
        completedUnripeMaterialOrders: 'Тасниф',
        consumed: 'Ишлатилди',
        createdAt: 'Сана',
        createdBy: 'Ким томонидан яратилган',
        status: 'Статус',
      },
      buttons: {
        add: 'Буюртма қўшиш',
        complete: 'Якунлаш'
      },
    },
    completedUnripeMaterialOrder: {
      header: {
        title: 'Якунланган Тўқув Буюртма',
        empty: 'Ҳеч қандай буюртма топилмади'
      },
      columns: {
        id: 'Буюртма ID',
        material: 'Материал номи',
        quantity: 'Миқдор',
        confirmedBy: 'Тасдиқланди',
        completedQuantity: 'Якунланган миқдор',
        completedQuantitySort2: 'Якунланган миқдор сорт 2',
        quantitySort2: 'Миқдор сорт 2',
        roll: 'Рўлон',
        completedRoll: 'Якунланган рўлон',
        completedRollSort2: 'Якунланган рўлон сорт 2',
        rollSort2: 'Рўлон сорт 2',
        expectedConsume: 'Кутилаётган сарф',
        consumed: 'Ишлатилди',
        createdBy: 'Ким томонидан яратилган',
        createdAt: 'Сана',
        status: 'Статус',
        completedUnripeMaterialOrders: 'Тасниф',
      }
    },
    modelOrder: {
      header: {
        title: 'Бичув Буюртмалари',
        empty: 'Ҳеч қандай буюртма топилмади'
      },
      columns: {
        description: 'Тасниф',
        productModel: 'Маҳсулот/Модел',
        createdAt: 'Сана',
        createdBy: 'Ким томонидан яратилди',
        productSize: 'Кутилаётган ўлчамлар',
        expectedOutlayRipeMaterial: 'Кутилаётган сарф',
        productModelOrderCompleteds: 'Тасниф',
        status: 'Статус',
      },
      buttons: {
        add: 'Буюртма қўшиш'
      },
    },
    completedProductModelOrder: {
      header: {
        title: 'Якунланган Бичув Буюртмалари',
        empty: 'Ҳеч қандай буюртма топилмади'
      },
      columns: {
        id: 'Буюртма ID',
        createdAt: 'Сана',
        createdBy: 'Ким томонидан яратилган',
        productModel: 'Модел',
        productSize: 'Модел размерлар',
        expectedOutlayRipeMaterial: 'Кутилаётган сарф',
        outlayRipeMaterial: 'Ишлатилди',
        status: 'Статус'
      }
    },
    warehouseAction: {
      header: {
        title: 'Маҳсулотлар Ҳаракати',
        empty: 'Ҳеч қандай ҳаракат топилмади'
      },
      columns: {
        id: 'Ҳаракат ID',
        createdAt: 'Сана',
        receivedAt: 'Қабул вақт',
        sentBy: 'Юборди',
        productModel: 'Модел',
        productSize: 'Модел размер',
        fromWarehouse: 'Дан',
        toWarehouse: 'Га',
        status: 'Статус'
      }
    },
    fabric: {
      header: {
        title: 'Бўёқ Фабрика',
        empty: 'Ҳеч қандай фабрика топилмади'
      },
      columns: {
        name: 'Ном',
        address: 'Манзил',
      },
      buttons: {
        add: 'Фабрика қўшиш'
      },
    },
    color: {
      header: {
        title: 'Ранглар',
        empty: 'Ҳеч қандай ранг топилмади'
      },
      columns: {
        name: 'Ном'
      },
      buttons: {
        add: 'Ранг қўшиш'
      },
    },
    model: {
      header: {
        title: 'Моделлар',
        empty: 'Ҳеч қандай модел топилмади'
      },
      columns: {
        name: 'Ном',
        description: 'Тасниф',
        sizes: 'Ўлчамлар',
        image: 'Расм'
      },
      buttons: {
        add: 'Модел қўшиш'
      },
    }
  },
  // Salary page translations
  salaryInfo: {
    title: 'Моаш ҳақида маълумот',
    types: {
      fixed: 'Белгиланган ойлик',
      piecework: 'Иш натижасига қараб',
      none: 'Маълумот йўқ'
    },
    tooltips: {
      fixed: {
        title: 'Белгиланган ойлик тизими',
        monthly: 'Ойлик моаш: Ҳар ой бир хил миқдорда тўланади',
        daily: 'Кунлик иш ҳақи: Ишлаган кунлар сони × кунлик тариф',
        general: 'Ойлик ёки кунлик тариф асосида ҳисобланади'
      },
      piecework: {
        title: 'Иш натижасига қараб ойлик',
        description: 'Бажарган ишлар миқдори ва нархи асосида ҳисобланади'
      }
    },
    cards: {
      totalEarned: {
        fixed: 'Ойлик моаши',
        piecework: 'Ишланган суммаси'
      },
      lastPayment: 'Сўнгги тўлов',
      bonus: 'Бонус',
      remaining: 'Қолган сумма',
      status: 'Ҳолат',
      paymentStatus: 'Тўлов ҳолати'
    },
    details: {
      monthlyRate: 'Ойлик тариф',
      dailyRate: '/кун',
      bonusAwarded: 'Ажойиб натижа! Мукофот берилди',
      debtStatus: 'Қарз ҳолати - тўланиши керак',
      noInfo: 'Ойлик маълумоти йўқ',
      notWorked: 'Ҳали иш бажарилмаган',
      allPaid: 'Барча тўловлар амалга оширилди'
    },
    statuses: {
      withBonus: 'Мукофот билан тўланган',
      fullyPaid: 'Тўлиқ ҳисобланган',
      partiallyPaid: 'Қисман тўланган',
      notPaid: 'Тўланмаган',
      inProgress: 'Жараёнда',
      noData: 'Маълумот йўқ'
    }
  },
  // DateFilter translations
  dateFilter: {
    thisPeriod: 'Бу давр'
  },
  // Month translations
  months: {
    january: 'Январ',
    february: 'Феврал',
    march: 'Март',
    april: 'Апрел',
    may: 'Май',
    june: 'Июн',
    july: 'Июл',
    august: 'Август',
    september: 'Сентябр',
    october: 'Октябр',
    november: 'Ноябр',
    december: 'Декабр'
  },
  unknown: 'Номаълум'
}
