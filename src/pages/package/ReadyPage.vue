...
const warehouse = ref([]);
const sendingWarehouse = ref('/api/warehouses/9');
...
const columns = [
  ...
  { name: 'receivedToPackageBy', label: t('checkedBy'), align: 'left', field: 'receivedToPackageBy' },
    ...
    <template v-slot:body="props">
      <q-tr :props="props">
        <q-td v-for="col in columns" :key="col.name" :props="props" :class="isToday(props.row.createdAt) && 'bg-green-1'">
          ...
          <div v-else-if="col.name === 'receivedToPackageBy'">
            {{ props.row?.receivedToPackageBy?.fullName || '-' }}
          </div>
